	...
	<div class="addEdit-block" :style='{"padding":"30px 0 0 280px"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' class="input" v-if="type!='info'"  label="体脂率" prop="tizhilv">
					<el-input v-model="ruleForm.tizhilv" placeholder="体脂率" clearable  :readonly="ro.tizhilv"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' v-else class="input" label="体脂率" prop="tizhilv">
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `tizhilv/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.tizhilvCrossAddOrUpdateFlag = false;
						 this.parent.search();
